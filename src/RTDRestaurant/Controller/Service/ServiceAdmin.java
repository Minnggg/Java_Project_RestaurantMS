...
public class ServiceAdmin {
    ...
    public ModelNhanVien getNV(int idNV) throws SQLException {
        ...
        String sql = "SELECT ID_NV, TenNV, DATE_FORMAT(NgayVL, '%d-%m-%Y') AS Ngay, SDT, ChucVu, ID_NQL, TinhTrang FROM NhanVien WHERE ID_NV = ?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, idNV);
        ResultSet r = p.executeQuery();
        while (r.next()) {
    ...
    public int getNextID_NV() throws SQLException {
        int id = 0;
        String sql = "SELECT MIN(ID_NV) + 1 AS nextID \n" +
                "FROM NhanVien \n" +
    ...
    public void insertNV(ModelNhanVien data) throws SQLException {
        String sql = "INSERT INTO NhanVien (ID_NV, TenNV, NgayVL, SDT, Chucvu, ID_NQL, TinhTrang) " +
                "VALUES (?, ?, STR_TO_DATE(?, '%d-%m-%Y'), ?, ?, ?, 'Dang lam viec')";
        PreparedStatement p = con.prepareStatement(sql);
    ...
    public int getRevenueHD(String filter) throws SQLException {
        ...
        if (filter.equals("Hôm nay")) {
            ...
            sql = "SELECT SUM(Tongtien) FROM HoaDon WHERE MONTH(NgayHD) = MONTH(CURDATE()) "
    ...
    public int getPreMonthRevenueHD() throws SQLException {
        int Pre_revenue = 0;
        String sql = "SELECT SUM(Tongtien) FROM HoaDon WHERE MONTH(NgayHD) = MONTH(CURDATE() - INTERVAL 1 MONTH) "
                + "AND YEAR(NgayHD) = YEAR(CURDATE() - INTERVAL 1 MONTH)";
        PreparedStatement p = con.prepareStatement(sql);
        ResultSet r = p.executeQuery();
        if (r.next()) {
    ...
    public int getPreMonthCostNK() throws SQLException {
        int Pre_Cost = 0;
        String sql = "SELECT SUM(Tongtien) FROM PhieuNK "
                + "WHERE MONTH(NgayNK) = MONTH(CURDATE() - INTERVAL 1 MONTH) "
                + "AND YEAR(NgayNK) = YEAR(CURDATE() - INTERVAL 1 MONTH)";
    ...
    public ArrayList<ModelChart> getRevenueCostProfit_byMonth() throws SQLException{
        ...
        String sql_Revenue = "SELECT MONTH(NgayHD) AS Thang, SUM(TONGTIEN) FROM HoaDon "
                + "WHERE YEAR(NgayHD) = YEAR(CURDATE()) "
                + "GROUP BY MONTH(NgayHD) ORDER BY Thang";

        String sql_Cost = "SELECT MONTH(NgayNK) AS Thang, SUM(TONGTIEN) FROM PhieuNK "
                + "WHERE YEAR(NgayNK) = YEAR(CURDATE()) "
